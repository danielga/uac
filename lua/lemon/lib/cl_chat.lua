local chat_prefixes = "-!"
net.Receive("lemon_chat_CPS", function(len)
	chat_prefixes = net.ReadString()
end)

net.Receive("lemon_chat_Text", function(len)
...
hook.Add("StartChat", "lemon.chat.StartChat", function()
	hook.Add("HUDPaint", "lemon.chat.HUDPaint", HUDPaint)
end)

hook.Add("FinishChat", "lemon.chat.FinishChat", function()
	hook.Remove("HUDPaint", "lemon.chat.HUDPaint")
end)

hook.Add("OnChatTab", "lemon.chat.OnChatTab", function(str)
	if chat_prefixes and autocomplete and #autocomplete > 0 and str:match(("^[%s][^ ]*$"):format(chat_prefixes)) then
		return autocomplete[1].C .. " "
	end
end)

hook.Add("ChatTextChanged", "lemon.chat.ChatTextChanged", function(text)
	if text == "" then
		autocomplete = nil
		return
	end
