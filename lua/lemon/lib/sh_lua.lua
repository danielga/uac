...
if SERVER then
	...
	net.Receive("lemon_lua", function(len)
		lemon.lua.Run(net.ReadString(), net.ReadString())
	end)
end

function lemon.lua.RunOnShared(src, by)
	lemon.lua.Run(src, by)
	if SERVER then
		lemon.lua.RunOnClients(src, by)
	else
		lemon.lua.RunOnServer(src)
	end
end
