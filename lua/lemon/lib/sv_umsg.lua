...
local function PrepareTable(name, ...)
	...
	for k, v in ipairs(tbl) do
		...
		if size >= maxsize or (size + itemsize > maxsize and itemtype ~= "string") then
			table.insert(newtbl[index], lemon.usermessage.UMSG_TYPE_STOP)
			index = index + 1
			newtbl[index] = {}
			table.insert(newtbl[index], name)
			size = 8 + namelen
		...
		if itemtype == "string" then
			if itemsize == 1 then
				if v ~= "" then
					table.insert(newtbl[index], lemon.usermessage.UMSG_TYPE_CHAR)
					table.insert(newtbl[index], string.byte(v))
				else
					table.insert(newtbl[index], lemon.usermessage.UMSG_TYPE_EMPTY_STRING)
				end
			elseif size - 1 > maxsize then
				local len = remain
				local str = string.sub(v, 1, len)
				table.insert(newtbl[index], lemon.usermessage.UMSG_TYPE_STRING)
				...
				while true do
					...
					len = templen
					table.insert(newtbl[index], lemon.usermessage.UMSG_TYPE_UNFINISHED_STRING)
					table.insert(newtbl[index], str)
		...
		elseif itemtype == "number" then
			table.insert(newtbl[index], lemon.usermessage.UMSG_TYPE_NUMBER)
			table.insert(newtbl[index], v)
		elseif itemtype == "Vector" then
			table.insert(newtbl[index], lemon.usermessage.UMSG_TYPE_VECTOR)
			table.insert(newtbl[index], v)
		elseif itemtype == "Angle" then
			table.insert(newtbl[index], lemon.usermessage.UMSG_TYPE_ANGLE)
			table.insert(newtbl[index], v)
		elseif itemtype == "boolean" then
			table.insert(newtbl[index], lemon.usermessage.UMSG_TYPE_BOOL)
			table.insert(newtbl[index], v)
		elseif itemtype == "Entity" or itemtype == "Player" or itemtype == "NPC" or itemtype == "Weapon" then
			table.insert(newtbl[index], lemon.usermessage.UMSG_TYPE_ENTITY)
			table.insert(newtbl[index], v)
		elseif itemtype == "PhysObj" then
			table.insert(newtbl[index], lemon.usermessage.UMSG_TYPE_PHYSOBJ)
			table.insert(newtbl[index], v)
		elseif IsColor(v) then
			table.insert(newtbl[index], lemon.usermessage.UMSG_TYPE_COLOR)
			table.insert(newtbl[index], v)
			size = size + itemsize + 1
		end
	end
	
	table.insert(newtbl[index], lemon.usermessage.UMSG_TYPE_END)
	return newtbl
end
	...
	for k, v in ipairs(tbl) do
		umsg.Start("le_umsg", recipient)
			...
			while true do
				...
				if itemtype == self.UMSG_TYPE_STRING or itemtype == self.UMSG_TYPE_UNFINISHED_STRING then
					umsg.String(v[i])
				elseif itemtype == self.UMSG_TYPE_EMPTY_STRING then
					i = i - 1
				elseif itemtype == self.UMSG_TYPE_CHAR then
					umsg.Char(v[i] - 128)
				elseif itemtype == self.UMSG_TYPE_NUMBER then
					umsg.Float(v[i])
				elseif itemtype == self.UMSG_TYPE_VECTOR then
					umsg.Vector(v[i])
				elseif itemtype == self.UMSG_TYPE_ANGLE then
					umsg.Angle(v[i])
				elseif itemtype == self.UMSG_TYPE_BOOL then
					umsg.Bool(v[i])
				elseif itemtype == self.UMSG_TYPE_ENTITY then
					umsg.Entity(v[i])
				elseif itemtype == self.UMSG_TYPE_COLOR then
				...
				elseif itemtype == self.UMSG_TYPE_PHYSOBJ then
				...
				elseif itemtype == self.UMSG_TYPE_STOP or itemtype == self.UMSG_TYPE_END then
					break
				end
