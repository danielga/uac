...
function uac.command.GetAutoComplete(command, args, showusage)
	args = args:match("^%s([^%s]+)")
...
concommand.Add("le", function(ply, command, args, str)
	...
	return uac.command.Run(ply, command, args)
end, function(command, args)
	return uac.command.GetAutoComplete(command, args)
end)

net.Receive("uac_command_ACS", function(len)
	auto_complete = {}
...
net.Receive("uac_command_ACA", function(len)
	auto_complete[net.ReadString()] = net.ReadString()
end)

net.Receive("uac_command_ACR", function(len)
