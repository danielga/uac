...
local function RetryConnection()
	local con = uac.sql.dbconnection
...
function uac.sql.Connect(recreate)
	...
	if (uac.config.GetValue("sql_connection_type") == "remote" or uac.config.GetValue("sql_connection_type") == "sourcebans") and not uac.config.GetBool("sql_redirected") then
		if recreate or uac.sql.dbconnection == nil then
			uac.sql.Initialized = false
			uac.sql.Retrying = false

			uac.sql.dbconnection = mysqloo.connect(uac.config.GetValue("sql_host"), uac.config.GetValue("sql_username"), uac.config.GetValue("sql_password"), uac.config.GetValue("sql_database"), uac.config.GetNumber("sql_host_port"))
			if uac.sql.dbconnection == nil then
				RetryConnection()
				return false
			end
			
			uac.sql.dbconnection.onConnected = ConnectSuccessCallback
			uac.sql.dbconnection.onConnectionFailed = ConnectFailureCallback
		end

		uac.sql.dbconnection:connect()
	else
		uac.sql.Initialized = true
		uac.sql.Retrying = false
	end
	...
	if uac.config.GetValue("sql_connection_type") == "remote" or uac.config.GetValue("sql_connection_type") == "sourcebans" then
		if uac.config.GetBool("sql_redirected") then
			...
			if uac.sql.Initialized and not uac.sql.Retrying then
				local databasequery = uac.sql.dbconnection:query(query)
