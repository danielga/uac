...
function PLUGIN:KickPlayer(ply, command, args)
	local targets = uac.player.GetTargets(ply, args[1], false)
...
function PLUGIN:Ban(ply, command, args)
	...
	elseif args[1]:sub(1, 6) == "STEAM_" then
		uac.ban.Add(args[1], time, reason)
	end
end
...
function PLUGIN:TrainBan(ply, command, args)
	...
	for i = 1, #targets do
		...
		train:SetEndCallback(function(train, ply, success)
			...
			uac.ban:Add(steamid, time, reason)
		end)
	end
end
...
function PLUGIN:TeleTo(ply, command, args)
	local target = uac.player.GetTargets(ply, args[1], false)
...
PLUGIN:AddHook("EntityTakeDamage", "UAC player control plugin (cancel damage on godded players)", PLUGIN.EntityTakeDamage)
...
PLUGIN:AddHook("PhysgunPickup", "UAC player control plugin (physgun players)", PLUGIN.PhysgunPickup)
...
PLUGIN:AddHook("PhysgunDrop", "UAC player control plugin (physgun players)", PLUGIN.PhysgunDrop)

function PLUGIN:PlayerNoclip(ply)
	if ply:GetUACTable().PhysgunPickup then return false end
end
PLUGIN:AddHook("PlayerNoclip", "UAC player control plugin (physgun players)", PLUGIN.PlayerNoclip)
--[[
...
PLUGIN:AddHook("Move", "UAC player control plugin (accelerating bunny hopping)", PLUGIN.Move)
]]
